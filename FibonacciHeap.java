...
public class FibonacciHeap {
    ...
    protected      int markedNodesCount = 0; // number of marked nodes at the moment
    ...
    * Returns the maximum possible rank of a tree in the heap
    ...
    public void decreaseKey(HeapNode x, int delta) {
        ...
        checkAndChangeMin(x); // update x to be the new minimum if needed
        if (x.isRoot() || x.getKey() >= x.parent.getKey()) { // heap order reserved, nothing to do
            return;
        }
        ...
        if (!curr.isRoot()) { // mark the last node in the process
            curr.markNode();
        }
